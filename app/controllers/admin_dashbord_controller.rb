class AdminDashbordController < ApplicationController
  ...
  def index
    ...
    @pending_learn_and_earns = LearnAndEarn.where(status: "pending").joins(:user).order(created_at: :desc).limit(10)

    # Add social tasks for quick overview
    @social_tasks = SocialTask.order(created_at: :desc).limit(5)

    # Get new buyer notifications
    @new_buyer_notifications = Notification.where(notification_type: "admin_alert", title: "New Buyer Registration").order(created_at: :desc).limit(10)
  end
end
